...
module.exports = function ($this) {
    ...
    main['add'] = function *() {
        ...
        var rules = {
                ...
                sign: {rule:'required',error:'验证失败!'},
                path: {rule:'required',error:'验证失败!'},
                memberId: {rule:'required',error:'验证失败!'}};
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $D('fileInfo').update($this.POST, {where: where});
                resData = $this.POST;
            } else {/*新增*/
                res = yield $D('fileInfo').build($this.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $D('fileInfo').findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $D('fileInfo').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $D('fileInfo').destroy({where: where});
        $this.success();
    };
